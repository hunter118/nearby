...
class PolymarketClient:
    ...
    def fetch_trades(
        ...
        while len(result) < limit:
            ...
            if response.status_code == 400 and "offset of 3000 exceeded" in response.text:
                break
            response.raise_for_status()
            chunk = response.json()
    ...
    def fetch_market_book(self, market_id: str) -> dict[str, Any]:
        response = self.session.get(
            f"{self.host}/book",
            params={"market": market_id},
            timeout=self.timeout_seconds,
    ...
    def normalize_market(raw: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "resolved_at": _parse_ts(raw.get("resolvedAt") or raw.get("resolveDate") or raw.get("endDate")),
    ...
    def normalize_trade(raw: dict[str, Any]) -> dict[str, Any]:
        ...
        if outcome_index in {0, "0"} and side in {"BUY", "BID"}:
            normalized_side = "BUY_YES"
        elif outcome_index in {1, "1"} and side in {"BUY", "BID"}:
            normalized_side = "BUY_NO"
        elif outcome_index in {0, "0"} and side in {"SELL", "ASK"}:
            normalized_side = "SELL_YES"
        elif outcome_index in {1, "1"} and side in {"SELL", "ASK"}:
        ...
        if normalized_side.endswith("NO") and raw.get("priceYes") is None:
            # Some endpoints report NO token price. Convert to YES probability price.
            price_yes = 1.0 - float(raw_price)

        return {
            "trade_id": str(raw.get("id") or raw.get("tradeID")),
